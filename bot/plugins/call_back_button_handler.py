...
from bot.locals.download_aria_p_n import aria_start
from bot.locals.youtube_dl_button import youtube_dl_call_back
from bot.plugins.status_message_fn import cancel_message_f
from bot import (
    MAX_MESSAGE_LENGTH,
    AUTH_CHANNEL
)
    ...
    if "|" in cb_data:
        ...
        if cb_data.startswith("cancel"):
            ...
            if update.from_user.id in AUTH_CHANNEL:
                g_d_list = ['app.json', 'venv', 'rclone.conf', '.gitignore', 'Dockerfile', 'DOWNLOADS', 'Procfile', '.heroku', '.profile.d', 'set.sh', 'bot', 'bot.log', 'vendor']
